...
@Tag(name = "教师接口",description = "关于教师的相关操作")
public class TeacherController {
    @Autowired
    private TeacherService teacherService;

    @Operation(summary = "教师的添加", description = "对教师进行添加")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "添加成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "teacher", description = "教师信息", required = true),
    })
    @PostMapping("/add")
    ...
    @Operation(summary = "教师的单个删除", description = "对教师进行单个删除")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "删除成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "id", description = "要删除教师的id", required = true),
    })
    ...
    @Operation(summary = "教师的批量删除", description = "对教师进行批量删除")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "删除成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "ids", description = "要删除教师的id列表", required = true),
    })
...
//
//    @Operation(summary = "教师的修改", description = "对教师进行修改")
//    @ApiResponses(value = {
//            @ApiResponse(responseCode = "200", description = "修改成功",
//                    content = @Content(schema = @Schema(implementation = String.class))),
//    })
//    @Parameters({
//            @Parameter(name = "teacher", description = "需要修改的教师信息", required = true),
//    })
//    @PutMapping("/update")
...
//    }
    ...
    @Operation(summary = "教师的id查询", description = "对教师进行id查询")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "查询成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "id", description = "要查询教师的id", required = true),
    })
    ...
    @Operation(summary = "教师的条件查询", description = "对教师进行条件查询")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "查询成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "teacher", description = "要查询教师的条件", required = true),
    })
    ...
    @Operation(summary = "教师的分页查询", description = "对教师进行分页查询")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "查询成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "params", description = "要查询教师的分页条件", required = true),
    })
    @GetMapping("/selectPage")
    public Result selectPage(Params params) {
        PageInfo<Teacher> page = teacherService.selectPage(params);
        return Result.success(page);
    }


    @Operation(summary = "教师的excel下载", description = "将教师进行打包，封装到excel中进行发送")
    @ApiResponses(value = {
    ...
    public Result export(HttpServletResponse response) throws IOException {
        ...
        List<Teacher> all = teacherService.selectAll(user);
        ...
        for (Teacher teacher : all) {
            ...
            row.put("角色", teacher.getRole() );
            row.put("姓名", teacher.getName());
            row.put("性别", teacher.getSex());
            row.put("电子邮箱", teacher.getEmail());
            row.put("电话", teacher.getPhone());
            row.put("账号", teacher.getAccount());
            row.put("密码", teacher.getPassword());
            row.put("密码摘要", teacher.getKeySm3());
            list.add(row);
        }
        ...
        response.setHeader("Content-Disposition","attachment;filename=TeacherInformation.xlsx");
    ...
    @Operation(summary = "教师的excel添加", description = "接受excel，映射到实体类，进行添加")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "教师添加成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
    ...
    @Operation(summary = "教师的单个重置密码", description = "通过id对教师进行重置密码，密码重置为123456")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "密码重置成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "id", description = "要重置密码的教师的id", required = true),
    })
    @PutMapping("/update/password/{id}")
    public Result updatePassword(@PathVariable Integer id){
        Teacher teacher=teacherService.selectById(id);
        teacherService.updatePd(teacher);
        return Result.success();
    }
    @Operation(summary = "教师的批量重置密码", description = "通过id列表对教师进行重置密码，密码重置为123456")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "密码重置成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "ids", description = "要重置密码的教师的id列表", required = true),
    })
    ...
    @Operation(summary = "教师的单个重置用户密钥", description = "通过id对教师进行重置密钥")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "密钥重置成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "id", description = "要重置密钥的教师的id", required = true),
    })
    @PutMapping("/update/key/{id}")
    public Result updateKey(@PathVariable Integer id){
        Teacher teacher=teacherService.selectById(id);
        teacherService.updateKey(teacher);
        return Result.success();
    }
    @Operation(summary = "教师的批量重置密钥", description = "通过id列表对教师进行重置密钥")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "密钥重置成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "ids", description = "要重置密钥的教师的id列表", required = true),
    })
    ...
    @Operation(summary = "教师进行个人信息的修改", description = "用户对自己的基本信息进行修改")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "信息修改成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    @Parameters({
            @Parameter(name = "teacher", description = "需要修改的用户的信息", required = true),
    })
    @PutMapping("/update/inform")
    public Result updateKey(@RequestBody Teacher teacher){
        teacherService.updateInform(teacher);
        return Result.success();
    }

    @Operation(summary = "教师进行密钥显示", description = "对用户的公钥和私钥进行显示")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "信息修改成功",
                    content = @Content(schema = @Schema(implementation = String.class))),
    })
    ...
    public Result getKey(@RequestBody Params params){
        ...
        //查找两对公钥和私钥
        String url1= RoleEnum.teacher.name()+"/"+params.getAccount()+"/user";
        String url2=RoleEnum.teacher.name()+"/"+params.getAccount()+"/exam";
