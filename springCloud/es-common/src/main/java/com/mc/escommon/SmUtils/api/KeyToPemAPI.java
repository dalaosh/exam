...
import com.mc.escommon.SmUtils.smutils.MySm2Util;
import com.mc.escommon.SmUtils.smutils.TxtUtil;
 ...
 * 将公钥和私钥转化为pem格式的
 */
public class KeyToPemAPI {
    public static ECPrivateKeyParameters privateKeyParameters;
    public static ECPublicKeyParameters publicKeyParameters;

    public KeyToPemAPI(String url) throws CertificateException, NoSuchAlgorithmException, InvalidKeySpecException, IOException, NoSuchProviderException {
        TxtUtil.readKey(url);
        ...
        publicKeyParameters = BCECUtil.convertPublicKeyToParameters(publicKey);
    }

    public KeyToPemAPI(String priKey,String pubKey) throws NoSuchAlgorithmException, InvalidKeySpecException, NoSuchProviderException {
    ...
    public List<String> pem() throws IOException {
        ...
        // 将公钥转换为PEM格式
        byte[] xCoord = publicKeyParameters.getQ().getAffineXCoord().getEncoded();
        byte[] yCoord = publicKeyParameters.getQ().getAffineYCoord().getEncoded();
