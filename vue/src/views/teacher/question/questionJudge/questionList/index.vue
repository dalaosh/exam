<template>
  <div class="teacher-page teacher-question-list-page">
    <section class="teacher-page-head">
      <div>
        <h2 class="teacher-page-head__title">判断题列表</h2>
    ...
    <section class="teacher-panel teacher-question-list-page__toolbar">
      ...
      <div class="teacher-question-list-page__actions">
        ...
        <el-upload action="http://localhost:9998/exam/questionJudge/upload" :show-file-list="false" :on-success="successUpload">
...
export default {
  ...
  methods: {
    ...
    load(){
      ...
      request.get('/exam/questionJudge/selectPage', {
        params:this.params
      }).then(res => {
        if (res.code === '200') {
    ...
    deleteById(id){
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/exam/questionJudge/delete/' + id).then(res => {
          if (res.code === '200') {
    ...
    deleteBatch(){
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/exam/questionJudge/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {
    ...
    insert() {
      this.$router.push("/teacher/teacherQuestionJudgeCreated")
    },
    updateMessage(id){
      this.$router.push({
        name: "teacherQuestionJudgeUpdate",
        params: {
          id:id
        }
      });
    ...
    exp() {
      location.href = 'http://localhost:9998/exam/questionJudge/export'
    },
    successUpload(res) {
      if (res.code === '200') {
