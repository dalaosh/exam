<template>
  <div class="teacher-page teacher-question-list-page">
    <section class="teacher-page-head">
      <div>
        <h2 class="teacher-page-head__title">简答题列表</h2>
    ...
    <section class="teacher-panel teacher-question-list-page__toolbar">
      <el-form :inline="true" :model="params" class="demo-form-inline teacher-question-list-page__filters">
        ...
        <el-form-item label="分值:">
          <el-input clearable v-model="params.score" placeholder="分值"></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <div class="teacher-question-list-page__actions">
        ...
        <el-upload action="http://localhost:9998/exam/questionShortAns/upload" :show-file-list="false" :on-success="successUpload">
...
export default {
  ...
  methods: {
    ...
    load(){
      ...
      request.get('/exam/questionShortAns/selectPage', {
        params:this.params
      }).then(res => {
        if (res.code === '200') {
    ...
    deleteById(id){
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/exam/questionShortAns/delete/' + id).then(res => {
          if (res.code === '200') {
    ...
    deleteBatch(){
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/exam/questionShortAns/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {
    ...
    insert() {
      this.$router.push("/teacher/teacherQuestionShortAnsCreated")
    },
    updateMessage(id){
      this.$router.push({
        name: "teacherQuestionShortAnsUpdate",
        params: {
          id:id
        }
      });
    ...
    showMessage(id){
      this.$router.push({
        name: "teacherQuestionShortAnsShow",
        params: {
          id:id
        }
      });
    ...
    exp() {
      location.href = 'http://localhost:9998/exam/questionShortAns/export'
    },
    successUpload(res) {
      if (res.code === '200') {
