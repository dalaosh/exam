  ...
  <div class="main">
    ...
    <div class="search">
      ...
      <el-row style="margin-top: 2vh;margin-left: 2%">
        ...
        <el-upload action="http://localhost:9998/user/student/upload" style="display: inline-block; margin-left: 10px" :show-file-list="false" :on-success="successUpload">
  ...
  methods: {
    ...
    deleteById(id){
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/user/student/delete/' + id).then(res => {
    ...
    updatePd(id){
      this.$confirm('您确定重置该密码吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/student/update/password/' + id).then(res => {
          if (res.code === '200') {   // 表示操作成功
    ...
    updateKey(id){
      this.$confirm('您确定重置该密钥吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/student/update/key/' + id).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('重置成功')
            this.findBySearch()
          } else {
    ...
    deleteBatch(){
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/user/student/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
    ...
    updatePdBatch(){
      ...
      this.$confirm('您确定批量重置密码吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/student/update/password/batch', this.ids).then(res => {
          if (res.code === '200') {   // 表示操作成功
    ...
    updateKeyBatch(){
      ...
      this.$confirm('您确定批量重置密钥吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/student/update/key/batch', this.ids).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('批量修改成功')
            this.findBySearch()
          } else {
