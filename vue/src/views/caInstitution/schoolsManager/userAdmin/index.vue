  ...
  <div class="main">
    ...
    <div class="search">
      ...
      <el-row style="margin-top: 2vh;margin-left: 2%">
        ...
        <el-button type="warning" @click="updatePdBatch" plain style="margin-right: 4%">批量重置密码</el-button>
        <el-button type="warning" @click="updateKeyBatch" plain style="margin-right: 4%">批量重置密钥</el-button>
        ...
        <el-upload action="http://localhost:9998/user/admin/upload" style="display: inline-block; margin-left: 10px" :show-file-list="false" :on-success="successUpload">
    ...
    <div class="table">
      ...
      >
        ...
        <el-table-column prop="id" label="管理员id" width="100" align="center"  fixed></el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-button type="text" @click="updatePd(scope.row.id)" size="small" style="color: #074ab0;">重置密码</el-button>
            <el-button type="text" @click="updateKey(scope.row.id)" size="small" style="color: #074ab0;">重置密钥</el-button>
...
export default {
  ...
  methods: {
    ...
    load(){
      ...
      request.get('/user/admin/selectPage', {
        params:this.params
      }).then(res => {
        if (res.code === '200') {
    ...
    deleteById(id){
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/user/admin/delete/' + id).then(res => {
    ...
    updatePd(id){
      this.$confirm('您确定重置该密码吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/admin/update/password/' + id).then(res => {
          if (res.code === '200') {   // 表示操作成功
    ...
    updateKey(id){
      this.$confirm('您确定重置该密钥吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/admin/update/key/' + id).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('重置成功')
            this.findBySearch()
          } else {
    ...
    deleteBatch(){
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/user/admin/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
    ...
    updatePdBatch(){
      ...
      this.$confirm('您确定批量重置密码吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/admin/update/password/batch', this.ids).then(res => {
          if (res.code === '200') {   // 表示操作成功
    ...
    updateKeyBatch(){
      ...
      this.$confirm('您确定批量重置密钥吗？', '确认重置', {type: "warning"}).then(response => {
        request.put('/user/admin/update/key/batch', this.ids).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('批量修改成功')
            this.findBySearch()
          } else {
    ...
    submit() {
      ...
      request({
        url: this.form.id ? '/user/admin/update' : '/user/admin/add',
    ...
    exp() {
      location.href = 'http://localhost:9998/user/admin/export'
    },
    successUpload(res) {
      if (res.code === '200') {
